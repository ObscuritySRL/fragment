...
/*
 * Shared x86 prologue length-decoder + relocation metadata -- the OS-independent
 * core of the inline-hook engine, used by both the Windows and Linux builds. The
 * instruction encoding does not change with the OS, so this lives once and a
 * decode fix lands for both ports at the same time.
 *
 * One source serves both widths: the includer sets FR_X86_BITS to 64 (x86-64,
 * via ../x86_64/decode.h) or 32 (IA-32, via ../i386/decode.h). They differ in
 * exactly three places -- REX prefixes (0x40-0x4F are INC/DEC reg in 32-bit, not
 * a prefix), RIP-relative addressing (a bare disp32 is absolute in 32-bit, so it
 * copies verbatim instead of being relocated), and the width a REX.W would have
 * widened a mov-immediate to -- each marked inline below; everything else, and
 * the prologue subset itself, is common.
 *
...
#ifndef FR_X86_BITS
#error "include ../x86_64/decode.h or ../i386/decode.h, not this core directly"
#endif
 ...
 * (relative to the ModRM byte) when the operand is RIP-relative, else -1. In
 * 32-bit a bare disp32 (mod=0,rm=5) is an absolute address, not PC-relative, so
 * it copies verbatim and *ripDisp stays -1. */
static int FrModRM(const uint8_t* m, int addr32, int* ripDisp) {
    *ripDisp = -1;
    ...
    if (rm == 4) {                            /* SIB present */
        ...
        if (mod == 0) {
            if (rm == 5) { if (FR_X86_BITS == 64 && !addr32) *ripDisp = len; len += 4; }  /* 64: RIP-rel; 32: abs disp32 */
        } else if (mod == 1) len += 1;        /* disp8  */
        else if (mod == 2) len += 4;          /* disp32 */
    }
    return len;
...
static int FrDecode(const uint8_t* p, FrInsn* in) {
...
#if FR_X86_BITS == 64
    if (p[i] >= 0x40 && p[i] <= 0x4F) { rexW = (p[i] & 8) != 0; i++; }  /* REX (32-bit: INC/DEC, handled below) */
#endif
#if FR_X86_BITS == 32
    if (asz) return 0;     /* 0x67 selects 16-bit addressing (a different ModRM
                            * table) -- outside the prologue subset; fail closed
                            * rather than mis-length. (In 64-bit 0x67 is 32-bit
                            * addressing, which the ModRM logic already handles.) */
#endif
...
#if FR_X86_BITS == 32
            if (osz) return 0;  /* 0x66 makes this a rel16 Jcc: outside the subset */
#endif
            in->relKind = 4; in->relOff = i; i += 4; return i;
        }
...
#if FR_X86_BITS == 32
    if (op >= 0x40 && op <= 0x4F) return i;                    /* inc/dec reg    */
#endif
    ...
    switch (op) {
        ...
        case 0x05: case 0x0D: case 0x15: case 0x1D: case 0x25: case 0x2D: case 0x35: case 0x3D: case 0xA9:
            i += osz ? 2 : 4; return i;
#if FR_X86_BITS == 32
        /* mov AL/eAX, moffs32 and back -- the absolute-address accumulator move a
         * 32-bit stack-protector prologue uses to load the gs:0x14 canary. The
         * moffs is address-sized (4 bytes; a 16-bit 0x67 override is refused
         * above) and absolute, so it copies verbatim. */
        ...
        case 0xE8: case 0xE9:
#if FR_X86_BITS == 32
            if (osz) return 0;  /* 0x66 makes this a rel16 call/jmp: outside the subset */
#endif
            in->relKind = 4; in->relOff = i; i += 4; return i;
